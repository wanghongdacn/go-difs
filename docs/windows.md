...
go get -u github.com/daccproject/go-difs
...
|`go get -u github.com/daccproject/go-difs`       |Fetch / Update `go-ipfs` source|
...
git clone https://github.com/daccproject/go-difs.git
cd %GOPATH%\src\github.com\ipfs\go-ipfs
make install
%GOPATH%\bin\ipfs.exe version --all
```
...
|`mkdir %GOPATH%\src\github.com\ipfs`<br/>`cd %GOPATH%\src\github.com\ipfs`<br/>`git clone https://github.com/daccproject/go-difs.git`       |Fetch / Update `go-ipfs` source|
|`cd %GOPATH%\src\github.com\ipfs\go-ipfs` |Change to `go-ipfs` source directory|
|`make install`                            |Build and install to `%GOPATH%\bin\ipfs.exe`|
|`%GOPATH%\bin\ipfs.exe version --all`     |Test the built binary|
...
```
go install -ldflags="-X "github.com/daccproject/go-difs".CurrentCommit=%SHA%"
%GOPATH%\bin\ipfs.exe version --all
```
...
If you encounter a bug with `go-ipfs` itself (not related to building) please use the [issue tracker](https://github.com/daccproject/go-difs/issues) to report it.
